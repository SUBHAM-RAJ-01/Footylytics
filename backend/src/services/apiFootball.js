...
// League ID mapping (API-Football uses different IDs)
const LEAGUE_MAP = {
...
// Convert API-Football match to our format
...
export const getUpcomingFixtures = async (leagueId) => {
  try {
    ...
    // Get next 30 days of fixtures
    const fixtures = await makeRequest('/fixtures', {
      league: apiLeagueId,
      season: season,
      next: 50 // Next 50 matches
    });
...
export const getStandings = async (leagueId) => {
  try {
    const apiLeagueId = LEAGUE_MAP[leagueId] || leagueId;
    logger.info(`Fetching standings for league ${leagueId} (API ID: ${apiLeagueId})`);
    ...
    const leagueStandings = standings[0].league.standings[0].map(team => ({
      position: team.rank,
      team: {
        id: team.team.id,
        name: team.team.name,
    ...
    logger.info(`Fetched standings for league ${leagueId}: ${leagueStandings.length} teams`);
    return { 
      standings: [{
